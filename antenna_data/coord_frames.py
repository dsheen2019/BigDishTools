...
From Sam The and converted to python for RSC-SIM
reverse transforms defined by dsheen 2026/01/21
"""
...
def cart_to_sphe_coord(x: float, y: float, z: float) -> np.ndarray:
    ...
    theta = np.arccos(z / r)
    phi = np.arctan2(y,x)  #np.sign(y) * np.arccos(x / np.sqrt(x**2 + y**2))
